...
Kafka.prototype.send = function (topic, messages, cb) {
    ...
    messages = Array.isArray(messages)
        ? messages.map(function (item) { return stringify(item); })
        : stringify(messages);
    producer.send([{
        topic: topic,
        messages: messages
...
setRemoting(Kafka.prototype.send, {
  ...
  accepts: [
    {arg: 'topic', type: 'String', description: 'Topic name', http: {source: 'query'} },
    {arg: 'messages', type: 'object', description: 'Message body', http: {source: 'body'}}
  ],
